...
public class RegionalPricingIndiaCalcRegPart extends BaseClass {
	...
	@Test
	public void regionalPricingIndiaCalcRegPart() throws EncryptedDocumentException, IOException, InterruptedException, ParseException {
		...
		System.out.println("Value of partOwner from UI is:" + locallySourcedCode);
		double codeValueNumeric = Integer.parseInt(locallySourcedCode);
		
		String calculatedPrice = LocalPartPricing.calculatedPrice().getText();
		System.out.println("Value of partOwner from UI is:"+calculatedPrice);
		double calculatedPriceNumeric = Integer.parseInt(calculatedPrice);
		
		String localCost = LocalPartPricing.localCost().getText();
		System.out.println("Value of partOwner from UI is:"+calculatedPrice);
		double localCostNumeric = Integer.parseInt(localCost);
		
		String resultingCost = LocalPartPricing.resultingCost().getText();
		System.out.println("Value of partOwner from UI is:"+resultingCost);
		double resultingCostNumeric = Integer.parseInt(resultingCost);
		
		String currentPrice = LocalPartPricing.currentPrice().getText();
		System.out.println("Value of partOwner from UI is:"+currentPrice);
		double currentPriceNumeric = Integer.parseInt(currentPrice);
		
		String publishPrice = LocalPartPricing.publishPrice().getText();
		System.out.println("Value of partOwner from UI is:"+publishPrice);
		double publishPriceNumeric = Integer.parseInt(publishPrice);
		...
		WebElement code = costRangeMultiplier.code();  
	...
	//	double codeValueNumeric = Integer.parseInt(codeValue);
		...
		WebElement multiplier = costRangeMultiplier.multiplier();  
		  ...
		  if(today.compareTo(futureEffectiveDate)>0) {
			...
			if(!currentPrice.isEmpty()) {
				   ...
				   segmententity.clickEntityname();
					...
					double mulOfCurrentPriceAndPricingActionIncreaseLimit = currentPriceNumeric* pricingActionIncreaselimitNumeric;
					double mulOfCurrentPriceAndPricingActionDecreaseLimit = currentPriceNumeric* pricingActionDecreaselimitValueNumeric;
					double mulOflocalCostNumericAndpricingActionValueNumeric =localCostNumeric *pricingActionValueNumeric;
					double mulOfresultingCostValueAndpricingActionValue = resultingCostNumeric * pricingActionValueNumeric;
					double mulOfcurrentPriceAndpricingActionValue = currentPriceNumeric * pricingActionValueNumeric;
					
				
					if(pricingActionMethodValue.equals(pricingActionMethod1) && !pricingActionValue.isEmpty() ) {  

						calculatedPriceNumeric = resultingCostNumeric* pricingActionValueNumeric;

					}
					else if(pricingActionMethodValue.equals(pricingActionMethod2) && !pricingActionValue.isEmpty()) {  
						
						calculatedPriceNumeric = currentPriceNumeric * pricingActionValueNumeric;
						...
						calculatedPriceNumeric = currentPriceNumeric * pricingActionIncreaselimitNumeric;
						...
						calculatedPriceNumeric = currentPriceNumeric * pricingActionDecreaselimitValueNumeric;

					}else if(pricingActionMethodValue.equals(pricingActionMethod4) && !localCost.equals("Null") ) {  

						 //( ldn.future price  OR ldn.publish price  OR ldn.current price) * (xref.Multipler OR xref_JV.Multiper)
						//calculatedPriceNumeric =futur * xRefMultiplierValueNumeric;

					}else if (pricingActionMethodValue.equals(pricingActionMethod4) && !publishPrice.equals("Null") ) {
						
						calculatedPriceNumeric = publishPriceNumeric * xRefMultiplierValueNumeric;
						
					}else if (pricingActionMethodValue.equals(pricingActionMethod4) && !currentPrice.equals("Null") ) {
						
						calculatedPriceNumeric = currentPriceNumeric * xRefMultiplierValueNumeric;
					}else if(!codeValue.isEmpty()){
						
						calculatedPriceNumeric = resultingCostNumeric * multiplierValueNumeric;
					}
